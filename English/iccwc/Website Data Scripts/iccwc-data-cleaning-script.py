...
file_path = r"D:\AI-ML\Data-Scraping-Wildlife-care\English\iccwc\Data Excels (Main Data, Shortened Excel)\iccwc-data.xlsx"
...
df_text_concat = df.groupby("unique_id", as_index=False).agg(
    {
        ...
        "article-post-images-src": lambda x: list(
            x.dropna()
...
for _, row in df_text_concat.iterrows():
    images = row["article-post-images-src"]
    if isinstance(images, list) and images:
        for i, image in enumerate(images):
            new_row = row.copy()
            new_row["unique_id"] = f"{row['unique_id']}-{i+1}"
            new_row["article-post-images-src"] = image
            if i > 0:
        ...
        row["article-post-images-src"] = None
        rows.append(row)
...
output_path = r"D:\AI-ML\Data-Scraping-Wildlife-care\English\iccwc\Data Excels (Main Data, Shortened Excel)\iccwc-data-cleaned.xlsx"
